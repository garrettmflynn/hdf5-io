...
export class LazyUint8Array {
  ...
  copyInto(
    ...
    while (i < length) {
      ...
      // const successCallback = this.callbacks.successCallback
  ...
  private getChunk(wantedChunkNum: number): Uint8Array {
    ...
    if (!this.cache.has(wantedChunkNum)) {
      ...
      // const progressCallback = this.callbacks.progressCallback
      // if (progressCallback) progressCallback(endByte/this.length, this.length, id)

    }
    if (!this.cache.has(wantedChunkNum)) throw new Error("doXHR failed (bug)!");
